...
- **Push right after committing.** Every successful commit on a feature branch is pushed to its upstream in the same step, without waiting for a separate request; the first push sets upstream. Never auto-push `master`; report pre-push hook or push failures instead of skipping or force-pushing.
